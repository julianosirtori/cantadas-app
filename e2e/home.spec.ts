...
const hasMatchPickUpLine = (pickUpLine: string | null) => data.pickUpLines.includes(pickUpLine?.trim().replaceAll("\"", '') || '')
...
test.describe('Cantadas', () => {
  ...
  test('should generate a new pick-up-line when click on generate Button', async ({ page }) => {
    const oldPickupLine = await page.locator('data-testid=text-pickUpLine').textContent();
    
    await page.locator('data-testid=button-generate').click();

    const newPickupLine = await page.locator('data-testid=text-pickUpLine').textContent();
  ...
  test('should copy pick-up-line when click on them', async ({ page }) => {
    ...
    await expect(page.locator('.Toastify__toast-body')).toHaveText('copiado');
  })
